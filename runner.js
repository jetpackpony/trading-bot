...
async function run() {
  ...
  const trader = await makeTrader(logId, postWindowSize * 60 * 1000, predictor);
...
const getStats = ({ closed, open }) => {
  ...
  return {
    ...
    totalProfitWithComission: R.sum(R.map(getProfitWithComission, deals))
  };
};

const comission = 0.0005;
const getProfitWithComission = (deal) => (
  (deal.sellPrice * (1 - comission)) / (deal.buyPrice * (1 + comission)) - 1
);
